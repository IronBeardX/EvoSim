    ...
    def instantiate_gene(self, mod_param_select: list):
        ...
        for i in range(len(mod_param_select)):
            if len(self.mod_list) != 0:
                specific_mod_list.append(self.mod_list[i](*mod_param_select[i]))
        gene_instance = { "name": self.name, "action": self.action, "mod_list": specific_mod_list, "action_des": self.action_decision }
...
class PerceptionGene(Gene):
    ...
    def instantiate_gene(self, mod_param_select: list):
        ...
        for i in range(len(mod_param_select)):
            if len(self.mod_list) != 0:
                specific_mod_list.append(self.mod_list[i](*mod_param_select[i]))
        gene_instance = { "name": self.name, "perception_action": self.perception_action, "mod_list": specific_mod_list }
        return (gene_instance, self.get_cost(gene_instance))
